...
from ete3.treeview.qt import (QGraphicsRectItem, QGraphicsLineItem,
...
class AttrFace(TextFace):
    ...
    def get_text(self):
        ...
        text = ''.join(map(str, [self.text_prefix, text, self.text_suffix]))
        return text
...
class ProfileFace(Face):
    ...
    def __init__(self, max_v, min_v, center_v, width=200, height=40, style="lines", colorscheme=2, values_vector=None,
                 deviations_vector=None):
        Face.__init__(self)
    ...
    def draw_bar_profile(self):
        ...
        for pos in range(vlength):
            ...
            if dev1 != 0:
                ...
                p.drawLine(
                    QLineF(1 + x_alpha / 2, profile_height - dev_up_y1, x1 + x_alpha / 2, profile_height - dev_down_y1))
                p.drawLine(QLineF(x1 - 1 + x_alpha / 2, profile_height - dev_up_y1, x1 + 1 + x_alpha / 2,
                                  profile_height - dev_up_y1))
                p.drawLine(QLineF(x1 - 1 + x_alpha / 2, profile_height - dev_down_y1, x1 + 1 + x_alpha / 2,
                                  profile_height - dev_down_y1))
        ...
        for pos in range(vlength):
            ...
            if dev1 != 0:
                if mean1 < self.center_v:
                    ...
                    p.drawLine(
                        QLineF(x1 + x_alpha / 2, mean_line_y + dev_up_y1, x1 + x_alpha / 2, mean_line_y + dev_down_y1))
                    p.drawLine(QLineF(x1 - 1 + x_alpha / 2, mean_line_y + dev_up_y1, x1 + 1 + x_alpha / 2,
                                      mean_line_y + dev_up_y1))
                    p.drawLine(QLineF(x1 - 1 + x_alpha / 2, mean_line_y + dev_down_y1, x1 + 1 + x_alpha / 2,
                                      mean_line_y + dev_down_y1))
                else:
...
class TreeFace(Face):
    ...
    def update_items(self):
        from ete3.treeview.qt4_render import render, init_tree_style
                    ...
                    if reg:
                        if not reg.startswith("-"):
                            if self.seq_format == "compactseq":
                                ...
                                motifs.append(
                                    [pos, pos + len(reg) - 1, self.seq_format, None, self.h, self.fgcolor, self.bgcolor,
                                     None])
                        pos += len(reg)
                ...
                for reg in re.split('([^-]+)', seq[current_seq_pos:start]):
                    if reg:
                        if reg.startswith("-") and self.seq_format != "seq":
                            self.regions.append(
                                [pos, pos + len(reg) - 1, self.gap_format, 1, 1, self.gapcolor, None, None])
                        else:
