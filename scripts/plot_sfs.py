...
def read_vcf(vcf, method, subsample, nbr_replicates, sift_file):
    dict_sift = open_sift(sift_file) if sift_file != "" else None
    ...
    for vcf_line in vcf_file:
        ...
        if "WS" in method and not ((dico_info["NUC_ANC"] in weak) and (dico_info["NUC_DER"] in strong)):
            continue

        if "SW" in method and not ((dico_info["NUC_ANC"] in strong) and (dico_info["NUC_DER"] in weak)):
            continue
        ...
        if dico_info["SNP_TYPE"] == "Syn":
            ...
            list_ns.append(SNP(snp_chr, snp_pos, s_sift, s_mutsel, s_omega, s_omega_0, count, k / sample_size))
    vcf_file.close()

    assert len(max_daf_set) == 1
    max_daf = max(max_daf_set)
    return list_ns, list_syn, max_daf


def classify_snps(list_ns, s_method, cat_snps):
    snp_dico = defaultdict(list)
    if cat_snps.bins:
        sorted_non_syn = sorted(list_ns, key=lambda x: getattr(x, s_method))
        for i, snp in enumerate(sorted_non_syn):
            snp_dico[f"bin{1 + int((i * cat_snps.nbr_non_syn()) / len(sorted_non_syn))}"].append(snp)
    else:
        for snp in list_ns:
            snp_dico[cat_snps.rate2cat(getattr(snp, s_method))].append(snp)
    return snp_dico


def plot_sift(list_ns, file):
    plt.figure(figsize=(1920 / my_dpi, 1080 / my_dpi), dpi=my_dpi)
    x = [snp.s_sift for snp in list_ns if snp.s_mutsel < 20]
    y = [snp.s_mutsel for snp in list_ns if snp.s_mutsel < 20]
...
def plot_density(list_ns, s_method, file):
    fig, ax = plt.subplots(figsize=(1920 / my_dpi, 960 / my_dpi), dpi=my_dpi)
    f_min = np.min([snp.frequency for snp in list_ns])
    assert f_min != 0
    n_lines = min(int(1 / f_min), 500)
    s_list = [getattr(snp, s_method) for snp in list_ns if np.isfinite(getattr(snp, s_method))]
    xmin, xmax = (-10, 5) if min(s_list) < 0.0 else (0, 1)
    x = np.linspace(xmin, xmax, 101)
    ys = list()
    f_cutoff_list = np.linspace(f_min, 1.0, n_lines)[::-1]
    for f_cut in f_cutoff_list:
        s_list = [getattr(snp, s_method) for snp in list_ns if
...
def plot_histogram(list_ns, cat_snps, s_method, file):
    fig, ax = plt.subplots(figsize=(1920 / my_dpi, 960 / my_dpi), dpi=my_dpi)
    s_list = [getattr(snp, s_method) for snp in list_ns if np.isfinite(getattr(snp, s_method))]
    xmin, xmax = (-10, 5) if min(s_list) < 0.0 else (0, 1)
...
def plot_venn(list_ns, method, cat_snps, snps_classified, file):
    from venn import venn
    cat_snps_mutsel = CategorySNP("MutSel", bins=cat_snps.bins)
    snps_classified_mutsel = classify_snps(list_ns, "s_mutsel", cat_snps_mutsel)
    ...
    for cat_i, cat_mutsel in enumerate(cat_snps_mutsel.non_syn()):
        for cat_j, cat in enumerate(cat_snps.non_syn()):
            dico_venn = {
                f"MutSel {cat_mutsel}": set([f"{snp.chr}_{snp.pos}" for snp in snps_classified_mutsel[cat_mutsel]]),
                f"{method} {cat}": set([f"{snp.chr}_{snp.pos}" for snp in snps_classified[cat]])}
...
def main(args):
    ...
    opp_dico = {cat: opp_results[cat].values[0] for cat in cat_snps.non_syn()}

    list_ns, list_syn, max_daf = read_vcf(args.vcf, args.method, args.subsample, args.nbr_replicates, args.sift_file)
    print(f'ratio pn/(ps + pn)={len(list_ns) / (len(list_syn) + len(list_syn))}')

    snps_classified = classify_snps(list_ns, s_method, cat_snps)
    plot_density(list_ns, s_method, args.output.replace('.pdf', '.distribution.pdf'))

    cat_dico_count = plot_histogram(list_ns, cat_snps, s_method, args.output.replace('.pdf', '.histogram.pdf'))
    cat_dico_count["syn"] = len(list_syn)

    if args.method != "MutSel":
        plot_venn(list_ns, args.method, cat_snps, snps_classified, args.output.replace('.pdf', '.venn.pdf'))
    if args.method == "SIFT":
        plot_sift(list_ns, args.output.replace('.SIFT.pdf', '.SIFT_vs_MutSel.pdf'))
    ...
    sfs_nonsyn_mean = np.mean(daf_to_sfs([snp.derived_count for snp in list_ns], max_daf), axis=0)
