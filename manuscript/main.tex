...
The detailed model implementation is available in~\nameref{S1_File}, described as a Bayesian hierarchical model (Fig~A in~\nameref{S1_File}).
...
Example substitutions in the terminal lineage of \textit{Chlorocebus sabaeus} which are classified as $\SphyBen$ are shown in~\nameref{S2_File} (section~1.1).
...
Among all the substitutions found in each terminal branch, between 10 and 13\% were $\SphyBen$, while $\SphyBen$ mutations only represent between 0.9 and 1.2\% of all non-synonymous mutations (Fig~\ref{fig:homo-afr-results}A-B for humans, Table~A in~\nameref{S2_File} for all dataset).
...
This rate is highly consistent across lineages (Table~A in~\nameref{S2_File}) and suggests an overall frequency of nearly neutral substitutions due to consistent long-term selection pressures, mutation and drift of approximately 20\% of all substitutions (20\%-26\% across species).
...
We obtained values between 1.17 and 1.75 in the different lineages (Table~B in~\nameref{S2_File}), meaning that $\SphyBen$ mutations reach fixation slightly more frequently than synonymous mutations that are supposed to be neutral, consistent with these $\SphyBen$ mutations being weakly beneficial.
...
Our results show that SNPs predicted as deleterious are associated with clinical terms such as \textit{Likely Pathogenic} and \textit{Pathogenic}, implying that, in general, the selective pressure of a mutation exerted across mammals is also predictive of its clinical effect in humans (Table~E in~\nameref{S2_File})~\cite{sullivan_leveraging_2023}.
...
Using correlation analyses that accounted for phylogenetic relationship (see section~\ref{subsec:correlation-diversity} in Materials \& Methods, Fig~E in~\nameref{S3_File}), we found that higher $\Ne$ was associated with a smaller proportion of nearly-neutral mutations ($r^2=0.31$, $p=0.001$, Fig~\ref{fig:diversity}A).
...
Third, we ran our analysis filtering out CpG mutations and obtained values of $\proba [ \SphyBen \given \SpopBen]$ in the range of 5--27\%, with a mean of 14\% (Table~B-C in~\nameref{S4_File}, Fig~D in~\nameref{S4_File}), providing more conservative estimates.
...
In this subset of the exome, containing genes with a more stable fitness landscape, we found an increase in the proportion of beneficial mutations that are non-adaptive (Wilcoxon signed-rank, $s=80$, $p=0.002$, Table~D in~\nameref{S4_File}), consistent with our expectation that beneficial mutations occur more frequently in genes under changing fitness landscapes.
...
These disjoint estimates are well correlated, with a goodness of fit $r^2={0.95, 0.89, 0.82}$ for respectively $\proba [\SpopDel]$ (Fig~G in~\nameref{S4_File}), $\proba [\SpopNeu]$(Fig~H in~\nameref{S4_File}) and $\proba [\SpopBen]$ (Fig~I in~\nameref{S4_File}).
...
If there are several substitutions toward the same amino acid along the mammalian tree (section~1.1 in~\nameref{S2_File}), our mutation-selection model cannot formally distinguish between a scenario where mammals have fixed deleterious mutations that are reverted in several lineages, from more complex scenarios involving convergent adaptation across mammals.
...
The priors and full configuration of the model are given in~\nameref{S1_File} (section~1).
...
\begin{equation}
L_{\text{syn}} = L_{\text{tot}} \frac{\mu_{\text{syn}}}{\mu_{\text{tot}}}.\label{eq:syn-opp}
\end{equation}
