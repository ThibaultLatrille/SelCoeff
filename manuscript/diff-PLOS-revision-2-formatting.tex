...
%DIF DEL main-PLOS-revision-2-plos2015.tex              Fri Dec  6 15:50:04 2024
%DIF ADD main-PLOS-revision-2-plos2015-formatting.tex   Mon Dec 16 22:08:01 2024
...
%DIF 89c89
%DIF < % Use nameref to cite supporting information files (see Supporting Information section for more info)
%DIF -------
...
\\
\DIFdelbegin \DIFdel{T. }\DIFdelend \DIFaddbegin \DIFadd{Thibault }\DIFaddend {Latrille}\textsuperscript{1\ddag*},
\DIFdelbegin \DIFdel{J. }\DIFdelend \DIFaddbegin \DIFadd{Julien }\DIFaddend {Joseph}\textsuperscript{2\ddag},
\DIFdelbegin \DIFdel{D.}\DIFdelend \DIFaddbegin \DIFadd{Diego}\DIFaddend ~A. {Hartasánchez}\textsuperscript{1},
\DIFdelbegin \DIFdel{N. }\DIFdelend \DIFaddbegin \DIFadd{Nicolas }\DIFaddend {Salamin}\textsuperscript{1}
\\
\bigskip
...
\DIFdelbegin \DIFdel{Detailed model description }\DIFdelend \DIFaddbegin \DIFadd{The detailed model implementation }\DIFaddend is available in~\nameref{S1_File}\DIFdelbegin \DIFdel{(}%DIFDELCMD < \nameref{S1_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{, described as a Bayesian hierarchical model (Fig~A in~}\nameref{S1_File}\DIFaddend ).
...
Example substitutions in the terminal lineage of \textit{Chlorocebus sabaeus} which are classified as $\SphyBen$ are shown in\DIFdelbegin \DIFdel{supplementary materials (sectionS2}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S2_File} \DIFadd{(section~1.1}\DIFaddend ).
For instance, in the mammalian protein-coding DNA alignment of gene \textit{SELE}, the nucleotide at site 1722 has mutated (from T to C) at the base of Simiiformes (monkeys and apes), modifying the corresponding amino acid from Serine to Proline, but has been subsequently reverted in the branch of \textit{Chlorocebus sabaeus} (\DIFdelbegin %DIFDELCMD < \nameref{S2_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~A in~}\nameref{S2_File}\DIFaddend ).
However, other substitutions classified as $\SphyBen$ in the terminal branch of \textit{Chlorocebus sabaeus} cannot be clearly interpreted as reversions along the terminal branch, and show several transitions to this amino acid across the mammalian phylogeny, as for instance site 3145 of gene \textit{THSD7A} (\DIFdelbegin %DIFDELCMD < \nameref{S3_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~B in~}\nameref{S2_File}\DIFaddend ).

Among all the substitutions found in each terminal branch, between 10 and 13\% were $\SphyBen$, while $\SphyBen$ mutations only represent between 0.9 and 1.2\% of all non-synonymous mutations (Fig~\ref{fig:homo-afr-results}A-B for humans\DIFdelbegin \DIFdel{and ~}%DIFDELCMD < \nameref{S1_Table} %%%
\DIFdelend \DIFaddbegin \DIFadd{, Table~A in~}\nameref{S2_File} \DIFaddend for all dataset).
...
This rate is highly consistent across lineages (\DIFdelbegin %DIFDELCMD < \nameref{S1_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~A in~}\nameref{S2_File}\DIFaddend ) and suggests an overall frequency of nearly neutral substitutions due to consistent long-term selection pressures, mutation and drift of approximately 20\% of all substitutions (20\%-26\% across species).
...
We obtained values between 1.17 and 1.75 in the different lineages (\DIFdelbegin %DIFDELCMD < \nameref{S2_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~B in~}\nameref{S2_File}\DIFaddend ), meaning that $\SphyBen$ mutations reach fixation slightly more frequently than synonymous mutations that are supposed to be neutral, consistent with these $\SphyBen$ mutations being weakly beneficial.
Such an observation is consistent with the premise that $\SphyBen$ mutations are weakly beneficial, translating to a scaled selection coefficient between 0.32 and 1.24 (\DIFdelbegin %DIFDELCMD < \nameref{S2_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{section~1.2 in~}\nameref{S2_File}\DIFaddend ).
...
By comparing these two ways of calculating $\dnds$ (see section~\ref{subsec:substitution-mapping-in-the-terminal-branch} in Materials \& Methods), we calculated that beneficial non-adaptive mutations inflate $\dnds$ values by between 9 and 12\% across genera \DIFaddbegin \DIFadd{(Table~C in~}\nameref{S2_File}\DIFadd{)}\DIFaddend .
This represents a substantial increase when considering that beneficial non-adaptive mutations only represent between 0.9 and 1.2\% of non-synonymous mutational opportunities (\DIFdelbegin %DIFDELCMD < \nameref{S3_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~A in~}\nameref{S2_File}\DIFaddend ).
...
First, SNPs classified as $\SphyBen$ are spread across the genomes and not strongly associated to the ontology terms of their respective genes (\DIFdelbegin %DIFDELCMD < \nameref{S4_Table}%%%
\DIFdel{,~}%DIFDELCMD < \nameref{S4_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~D and Fig~C in~}\nameref{S2_File}\DIFaddend ).
...
Our results show that SNPs predicted as deleterious are associated with clinical terms such as \textit{Likely Pathogenic} and \textit{Pathogenic}, implying that, in general, the selective pressure of a mutation exerted across mammals is also predictive of its clinical effect in humans (\DIFdelbegin %DIFDELCMD < \nameref{S5_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~E in~}\nameref{S2_File}\DIFaddend )~\cite{sullivan_leveraging_2023}.
Conversely, $\SphyBen$ mutations are associated with clinical terms such as \textit{Benign} and \textit{Likely Benign}, which shows that $\SphyBen$ mutations are less likely to be functionally damaging (\DIFdelbegin %DIFDELCMD < \nameref{S6_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~F in~}\nameref{S2_File}\DIFaddend ).
...
From these estimated fitness effects, we can derive the proportion of deleterious mutations ($\ProbaPopDel$\DIFdelbegin \DIFdel{,~}%DIFDELCMD < \nameref{S5_Fig}%%%
\DIFdelend ), nearly-neutral mutations ($\ProbaPopNeu$\DIFdelbegin \DIFdel{,~}%DIFDELCMD < \nameref{S6_Fig}%%%
\DIFdelend ) and beneficial mutations ($\ProbaPopBen$\DIFdelbegin \DIFdel{,~}%DIFDELCMD < \nameref{S7_Fig}%%%
\DIFdelend ) at the population scale (see section~\ref{subsec:s-polymorphism-method} in Materials \& Methods\DIFaddbegin \DIFadd{, Fig~A-C in~}\nameref{S3_File}\DIFaddend ).
These approaches offer a unique opportunity to contrast selection coefficients estimated at the phylogenetic scale ($\Sphy$) and at the population scale ($\Spop$) in different dataset (\DIFdelbegin %DIFDELCMD < \nameref{S8_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~D in~}\nameref{S3_File}\DIFaddend ).
...
Using correlation analyses that accounted for phylogenetic relationship (see section~\ref{subsec:correlation-diversity} in Materials \& Methods, \DIFdelbegin \DIFdel{~}%DIFDELCMD < \nameref{S9_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~E in~}\nameref{S3_File}\DIFaddend ), we found that higher $\Ne$ was associated with a smaller proportion of nearly-neutral mutations ($r^2=0.31$, $p=0.001$, Fig~\ref{fig:diversity}A).
This result follows the prediction of the nearly-neutral theory and suggests that in populations with higher diversity (e.g.,~\textit{Bos} or \textit{Ovis}), discrimination between beneficial and deleterious mutations is more likely to occur (\DIFdelbegin %DIFDELCMD < \nameref{S10_Fig}%%%
\DIFdel{,~}%DIFDELCMD < \nameref{S11_Fig}%%%
\DIFdel{,~}%DIFDELCMD < \nameref{S12_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~F-H in~}\nameref{S3_File}\DIFaddend ).
...
Importantly, the \textit{recall} value in this case, computed as $\proba [ \SphyBen \given \SpopBen]$, is the probability for a beneficial mutation at the population scale to be a non-adaptive, i.e., going toward a fitter amino acid given a stable fitness landscape (Fig~\ref{fig:method}G, \DIFdelbegin \DIFdel{~}%DIFDELCMD < \nameref{S7_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~A in~}\nameref{S4_File}\DIFaddend ).
...
First, we controlled that these estimations were not affected by SNP mispolarization (\DIFdelbegin %DIFDELCMD < \nameref{S13_Fig}%%%
\DIFdel{,~}%DIFDELCMD < \nameref{S14_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~A-B in~}\nameref{S4_File}\DIFaddend ).
Second, we performed simulations at the population-genetic level and confirmed that our method was able to recover the proportion of beneficial mutations that are non-adaptive in synthetic polymorphism datasets (\DIFdelbegin %DIFDELCMD < \nameref{S15_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~C in~}\nameref{S4_File}\DIFaddend ).
Third, we ran our analysis filtering out CpG mutations and obtained values of $\proba [ \SphyBen \given \SpopBen]$ in the range of 5--27\%, with a mean of 14\% (\DIFdelbegin %DIFDELCMD < \nameref{S8_Table}%%%
\DIFdel{, }%DIFDELCMD < \nameref{S9_Table}%%%
\DIFdel{, }%DIFDELCMD < \nameref{S16_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~B-C in~}\nameref{S4_File}\DIFadd{, Fig~D in~}\nameref{S4_File}\DIFaddend ), providing more conservative estimates.
...
In this subset of the exome, containing genes with a more stable fitness landscape, we found an increase in the proportion of beneficial mutations that are non-adaptive (Wilcoxon signed-rank, $s=80$, $p=0.002$, \DIFdelbegin \DIFdel{~}%DIFDELCMD < \nameref{S10_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~D in~}\nameref{S4_File}\DIFaddend ), consistent with our expectation that beneficial mutations occur more frequently in genes under changing fitness landscapes.
...
Hence, we estimated \textit{precision} and \textit{recall} as done previously, but now including the number of substitutions per site as input for the DFE estimators (see~\ref{subsec:s-polymorphism-method} in Materials \& Methods and \DIFdelbegin \DIFdel{~}%DIFDELCMD < \nameref{S17_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~E in~}\nameref{S4_File}\DIFaddend ).
When including substitutions in the terminal lineage, estimates of $\proba [ \SphyBen \given \SpopBen]$ are in the 10--78\% range with a mean of 36\%, and 19 out of 28 estimates fall between 15\% and 45\% (\DIFdelbegin %DIFDELCMD < \nameref{S11_Table}%%%
\DIFdel{, }%DIFDELCMD < \nameref{S12_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~E-F in~}\nameref{S4_File}\DIFaddend ).

Additionally, we controlled that these estimations were not affected by SNP mispolarization (\DIFdelbegin %DIFDELCMD < \nameref{S18_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~F in~}\nameref{S4_File}\DIFaddend ).
We also filtered out genes under pervasive adaptation, and again found an increase in $\proba [ \SphyBen \given \SpopBen]$, consistent with our expectation (Wilcoxon signed-rank, $s=120$, $p=0.027$, \DIFdelbegin \DIFdel{~}%DIFDELCMD < \nameref{S13_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~G in~}\nameref{S4_File}\DIFaddend ).
...
These disjoint estimates are well correlated, with a goodness of fit $r^2={0.95, 0.89, 0.82}$ for respectively $\proba [\SpopDel]$ (\DIFdelbegin %DIFDELCMD < \nameref{S19_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~G in~}\nameref{S4_File}\DIFaddend ), $\proba [\SpopNeu]$(\DIFdelbegin %DIFDELCMD < \nameref{S20_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~H in~}\nameref{S4_File}\DIFaddend ) and $\proba [\SpopBen]$ (\DIFdelbegin %DIFDELCMD < \nameref{S21_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~I in~}\nameref{S4_File}\DIFaddend ).
...
Instead of using such a continuous DFE, we also tested our prediction with a non-parametric functional form for the DFE, obtaining estimates of $\proba [ \SphyBen \given \SpopBen]$ in the 8--94\% range, with a mean of 43\% (\DIFdelbegin %DIFDELCMD < \nameref{S14_Table}%%%
\DIFdel{, }%DIFDELCMD < \nameref{S15_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~H-I in~}\nameref{S4_File}\DIFaddend ).
...
However, contrary to SIFT scores, our mutation-selection model is parameterized by a fitness function such that changes are directly interpretable as fitness effects (see also section\DIFdelbegin \DIFdel{S5}\DIFdelend \DIFaddbegin \DIFadd{~1 in~}\nameref{S3_File}\DIFaddend ).
...
If there are several substitutions toward the same amino acid along the mammalian tree (\DIFdelbegin %DIFDELCMD < \nameref{S3_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{section~1.1 in~}\nameref{S2_File}\DIFaddend ), our mutation-selection model cannot formally distinguish between a scenario where mammals have fixed deleterious mutations that are reverted in several lineages, from more complex scenarios involving convergent adaptation across mammals.
...
Importantly, we would usually expect adaptive convergent mutations to be linked to particular converging phenotypes across mammals, and hence, they should not massively affect the whole genome as we find (\DIFdelbegin %DIFDELCMD < \nameref{S4_Table}%%%
\DIFdel{,~}%DIFDELCMD < \nameref{S4_Fig}%%%
\DIFdelend \DIFaddbegin \DIFadd{Fig~C and Table~D in~}\nameref{S2_File}\DIFaddend ).
Moreover, after filtering out genes usually associated to recurrent adaptation (e.g. immune genes), we recover an even higher proportion of beneficial non-adaptive mutations (\DIFdelbegin %DIFDELCMD < \nameref{S8_Table}%%%
\DIFdelend \DIFaddbegin \DIFadd{Table~D in~}\nameref{S4_File}\DIFaddend ).
...
The priors and full configuration of the model are given in\DIFdelbegin \DIFdel{section S1}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S1_File} \DIFadd{(section~1)}\DIFaddend .
...
\begin{equation}
L_{\text{syn}} = L_{\text{tot}} \frac{\mu_{\text{syn}}}{\mu_{\text{tot}}}.\DIFaddbegin \label{eq:syn-opp}
\DIFaddend \end{equation}
...
The mammalian dated tree was obtained from TimeTree~\cite{kumar_timetree_2017} and pruned to include only the species analysed in this study, with multi-furcation of the different populations from each species placed at the same divergence time as the species (section\DIFdelbegin \DIFdel{S4.1}\DIFdelend \DIFaddbegin \DIFadd{~2.1 in~}\nameref{S3_File}\DIFaddend ).

\DIFdelbegin \section*{\DIFdel{Data availability}}
%DIFAUXCMD
\DIFdel{The data underlying this article are available at }%DIFDELCMD < \url{https://doi.org/10.5281/zenodo.7878953}%%%
\DIFdel{.
Snakemake pipeline, analysis scripts and documentation are available at }\href{https://github.com/ThibaultLatrille/SelCoeff}{\DIFdel{github.com/ThibaultLatrille/SelCoeff}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < 
...
Project management and funding: N.S}%DIFDELCMD < \@%%%
\DIFdel{.
}%DIFDELCMD < 
...
\paragraph*{S1 File.}
\label{S1_File}
{\bf Supplementary appendix \DIFaddbegin \DIFadd{on the parameterization of Mutation-selection codon models}\DIFaddend .} Contains \DIFdelbegin \DIFdel{43 }\DIFdelend \DIFaddbegin \DIFadd{5 }\DIFaddend pages of supplementary information \DIFdelbegin \DIFdel{: model parameterization, supplementary figures (S1 Fig to S21 Fig)and supplementary tables (S1 Table to S15 Table)}\DIFdelend \DIFaddbegin \DIFadd{including 1 figure (Fig A)}\DIFaddend .

\DIFdelbegin \subsection*{\DIFdel{List of Figures}}
%DIFAUXCMD
\DIFdelend \paragraph*{\DIFdelbegin \DIFdel{S1 Fig}\DIFdelend \DIFaddbegin \DIFadd{S2 File}\DIFaddend .}
\DIFdelbegin %DIFDELCMD < \label{S1_Fig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{S2_File}
\DIFaddend {\bf \DIFdelbegin \DIFdel{Bayesian hierarchical model.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S2 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S2_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Examples of $\bm{\SphyBen}$ mutation sites in }\textit{\DIFdel{Chlorocebus sabaeus}} %DIFAUXCMD
\DIFdel{(reversions).}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S3 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S3_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Examples of $\bm{\SphyBen}$ mutation sites in }\textit{\DIFdel{Chlorocebus sabaeus}}%DIFAUXCMD
\DIFdel{.}%DIFDELCMD < }
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\paragraph*{\DIFdel{S4 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S4_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Enrichment in $\bm{\SphyBen}$ SNPs across all gene ontology terms.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S5 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S5_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Estimation of selection at the population scale for $\bm{\SphyDel}$ mutations.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S6 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S6_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Estimation of selection at the population scale for $\bm{\SphyNeu}$ }\DIFdelend \DIFaddbegin \DIFadd{Supplementary appendix on the caracterization of non-adaptive beneficial }\DIFaddend mutations.} \DIFaddbegin \DIFadd{Contains 12 pages of supplementary information including 3 figures (Fig A to C) and 6 tables (Table A to F).
}\DIFaddend 

\DIFdelbegin \paragraph*{\DIFdel{S7 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S7_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Estimation of selection at the population scale for $\bm{\SphyBen}$ mutations.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S8 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S8_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Distribution of scaled selection coefficients ($\bm{\Sphy}$) .}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S9 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S9_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Tree used for PGLM (Phylogenetic Generalized Linear Model).
}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \paragraph*{\DIFdelbegin \DIFdel{S10 Fig}\DIFdelend \DIFaddbegin \DIFadd{S3 File}\DIFaddend .}
\DIFdelbegin %DIFDELCMD < \label{S10_Fig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{S3_File}
\DIFaddend {\bf \DIFdelbegin \DIFdel{Relationship between the proportion of deleterious mutations ($\bm{\SpopDel}$) and effective population size.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S11 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S11_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Relationship between the proportion of deleterious mutations ($\bm{\SpopNeu}$) and effective population size}\DIFdelend \DIFaddbegin \DIFadd{Supplementary appendix on the contrast of selection at the phylogenetic and population-genetic scales}\DIFaddend .} \DIFaddbegin \DIFadd{Contains 7 pages of supplementary information including 8 figures (Fig A to H).
}\DIFaddend 

\DIFdelbegin \paragraph*{\DIFdel{S12 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S12_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Relationship between the proportion of deleterious mutations ($\bm{\SpopBen}$)and effective population size.
}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \paragraph*{\DIFdelbegin \DIFdel{S13 Fig}\DIFdelend \DIFaddbegin \DIFadd{S4 File}\DIFaddend .}
\DIFdelbegin %DIFDELCMD < \label{S13_Fig}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{S4_File}
\DIFaddend {\bf \DIFdelbegin \DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ as a function of distance to the sister species.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S14 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S14_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Ratio of $\bm{\proba[\Sphy]}$ over $\bm{\proba[-\Sphy]}$ for polymorphisms of the African population of }\textit{\DIFdel{Homo sapiens}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{Supplementary appendix on controls for estimating the proportion of beneficial mutations that are not adaptive}\DIFaddend .} \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S15 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S15_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Proportion of non-adaptive mutations among beneficial ones ($\bm{\proba{[}\SphyBen\given \SpopBen {]}}$) estimated on synthetic site-frequency spectra.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S16 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S16_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Site-frequency spectrum when masking CpG transitions.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S17 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S17_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{Including substitutions in the terminal lineage to estimate $\bm{\Spop}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S18 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S18_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when including divergence to estimate $\bm{\Spop}$, as a function of distance to the sister species.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S19 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S19_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba [ \SpopDel ]}$ when fitting a single or three DFEs.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S20 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S20_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba [ \SpopNeu ]}$ when fitting a single or three DFEs.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S21 Fig.}}
%DIFAUXCMD
%DIFDELCMD < \label{S21_Fig}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba [ \SpopBen ]}$ when fitting a single or three DFEs.}%DIFDELCMD < }
%DIFDELCMD < 
...
%DIFAUXCMD
%DIFDELCMD < \label{S1_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Probability of mutations and substitutions to be $\bm{\SphyDel}$, $\bm{\SphyNeu}$ or $\bm{\SphyBen}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S2 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S2_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\dnds}$ for $\bm{\SphyDel}$, $\bm{\SphyNeu}$ or $\bm{\SphyBen}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S3 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S3_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\dnds}$ over-estimation due to non-adaptive beneficial mutations.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S4 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S4_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Gene ontology enrichment in $\bm{\SphyBen}$ SNPs.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S5 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S5_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Terms associated with deleterious mutations $\bm{\SphyDel}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S6 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S6_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Terms associated with non-adaptive beneficial mutations $\bm{\SphyBen}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S7 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S7_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Probability for beneficial mutations to be non-adaptive ($\bm{\proba[\SphyBen\given \SpopBen ]}$).}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S8 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S8_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when masking CpG transitions.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S9 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S9_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Precision and recall when masking CpG transitions.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S10 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S10_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ excluding or not genes under adaptation.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S11 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S11_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when including divergence to estimate $\bm{\Spop}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S12 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S12_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Precision and recall when including divergence to estimate $\bm{\Spop}$.
}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S13 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S13_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ excluding or not genes under adaptation, when including divergence to estimate $\bm{\Spop}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S14 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S14_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when using a discrete DFE to estimate $\bm{\Spop}$.}%DIFDELCMD < }
%DIFDELCMD < 

%DIFDELCMD < %%%
\paragraph*{\DIFdel{S15 Table.}}
%DIFAUXCMD
%DIFDELCMD < \label{S15_Table}
%DIFDELCMD < {\bf %%%
\DIFdel{Precision and recall when using a discrete DFE to estimate $\bm{\Spop}$.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Contains 21 pages of supplementary information including 9 figures (Fig A to I) and 9 tables (Table A to I).
}\DIFaddend 
