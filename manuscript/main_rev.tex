...
\begin{document}
    ...
    Adaptation is considered as the main process driving the diversity of forms and functions across the tree of life\cite{darwin_origin_1859}, and it enables species to live in new ecological niches, and to evolve in response to a change in environment\cite{darwin_origin_1859, merrell_adaptive_1994}.
    ...
    With the discovery of DNA, the main support of heritable information, evolutionary geneticists looked for signatures of adaptation in genomes, and tried to quantify it\cite{mcdonald_adaptative_1991, eyre-walker_distribution_2006}.
    ...
    A way to measure maintenance positive selection is to predict selection coefficients assuming a constant and stable fitness landscape estimated from the alignment of protein-coding genes.
    ...
    First, we know that a lot of mechanisms can influence amino acid frequency other than fitness (phylogenetic inertia, mutation biases, GC-biased gene conversion, mutational paths) which are not accounted for in SIFT\@.
    ...
    Are these polymorphisms effectively under positive selection along the terminal branches ? At the scale of populations ?
    ...
    For each non-synonymous SNP observed at the population level, we predicted its scaled selection coefficient ($\Sphy$) from a stable fitness landscape estimated at the mammalian scale (fig.~\ref{fig:method}B).
    ...
    For example, if a variant predicted to be non-optimal is found to be fixed in the last common ancestor of extant humans. If a mutation occurs in extant humans towards the allele predicted to be optimal, is this mutation positively selected at the population-scale?
    ...
    In each of theses classes, SNPs are then grouped by the frequency of derived alleles in the population (fig.~\ref{fig:homo-afr-results}C), generating a so called site-frequency spectrum (SFS).
    ...
    We replicated such experiment across 28 populations from 6 species(fig.~\ref{fig:heatmap}), and found that mutations predicted to be deleterious are indeed purged at the population-scale (blue and green rows in fig.~\ref{fig:heatmap}A).
    ...
    Since $\Ne$ is not directly measurable, a reasonable proxy usually used is the neutral diversity (Watterson $\theta_w$).
    Assuming that synonymous positions are effectively neutral in mammals, we thus ordered populations by their levels of synonymous diversity (fig.~\ref{fig:diversity}A) and confirmed that higher synonymous diversity is typically accompanied by a smaller proportion of neutral mutations.
    ...
    For each population with available polymorphism data, we first reconstructed the ancestral sequence by replacing all variable site of the reference genome with the ancestral allele predicted by est-usfs, then we mapped the substitutions from the last genus split to this reconstructed ancestral sequence.
    ...
    Altogether, the probability for a mutation to be restoring the genome given that it is positively selected is obtained for each population, and varies between 10\% and 26\% depending of the population (table S2).
