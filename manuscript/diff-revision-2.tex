...
%DIF DEL main-revision.tex     Sun Oct 15 00:20:50 2023
%DIF ADD main-revision-2.tex   Mon Mar  4 11:43:41 2024
%TC:ignore
...
%TC:endignore
    \begin{abstract}
        Mutations can be beneficial by bringing innovation to their bearer, allowing them to adapt to environmental change. These mutations are typically unpredictable since they respond to an unforeseen change in the environment. However, mutations can also be beneficial because they are simply restoring \DIFdelbegin \DIFdel{an ancestral sequence }\DIFdelend \DIFaddbegin \DIFadd{a state }\DIFaddend of higher fitness that was lost due to genetic drift \DIFaddbegin \DIFadd{in a stable environment}\DIFaddend . In contrast to adaptive mutations, these beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend can be predicted if the underlying fitness landscape is stable and known. The contribution of such \DIFdelbegin \DIFdel{beneficial back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend to molecular evolution has been widely neglected mainly because their detection is very challenging. We have here reconstructed protein-coding-gene fitness landscapes shared between mammals, using mutation-selection models and a multi-species alignments across 87 mammals. These fitness landscapes have allowed us to predict the fitness effect of polymorphisms found in 28 mammalian populations. Using methods that quantify selection at the population level, we have confirmed that beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend are indeed positively selected in extant populations. Our work confirms that deleterious substitutions are accumulating in mammals and are being reverted, generating a balance in which genomes are damaged and restored simultaneously at different loci. We observe that beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend represent between 15\% and 45\% of all beneficial mutations in 24 of 28 populations analyzed, showing that a substantial part of ongoing positive selection is not driven \DIFaddbegin \DIFadd{solely }\DIFaddend by adaptation to environmental change in mammals.
    \end{abstract}

    \DIFdelbegin %DIFDELCMD < \keywords{Adaptation \and Back-mutations \and Phylogenetics \and Population-genetics \and Codon models }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \keywords{Adaptation \and Nearly-neutral \and Phylogenetics \and Population-genetics \and Codon models }
\DIFaddend 
    ...
    \DIFaddbegin \subsection*{\DIFadd{Beneficial yet non-adaptive mutations}}

    \DIFaddend In a constant environment, a deleterious mutation can reach fixation by genetic drift~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
...
\cite{ohta_nearly_1992}}\hskip0pt%DIFAUXCMD
    ...
    \DIFdelbegin \DIFdel{The restoration of }\DIFdelend \DIFaddbegin \DIFadd{We will refer to as beneficial non-adaptive mutations to those mutations that restore }\DIFaddend the ancestral fitness \DIFdelbegin \DIFdel{can either }\DIFdelend \DIFaddbegin \DIFadd{under the assumption that the fitness landscape has not changed~\mbox{%DIFAUXCMD
\cite{piganeau_estimating_2003, charlesworth_other_2007}}\hskip0pt%DIFAUXCMD
.
    Such mutations can }\DIFaddend happen through a mutation at a different locus\DIFdelbegin \DIFdel{–~}\DIFdelend \DIFaddbegin \DIFadd{, in which case it is }\DIFaddend called a compensatory mutation~\cite{hartl_compensatory_1996, mustonen_fitness_2009}\DIFdelbegin \DIFdel{, or at the locus of the initial mutation –~called a beneficial back-mutation~\mbox{%DIFAUXCMD
\cite{piganeau_estimating_2003, charlesworth_other_2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend .
    While compensatory mutations change the sequence and thus induce genetic diversification, beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations at the locus of the initial mutation }\DIFaddend reduce genetic diversity and do not contribute to genetic innovation\DIFaddbegin \DIFadd{, which are the focus of this manuscript}\DIFaddend .
    Although Tomoko Ohta considered beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend negligible in her nearly-neutral theory~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ohta1992}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{ohta_nearly_1992}}\hskip0pt%DIFAUXCMD
}\DIFaddend , their importance has now been acknowledged for expanding populations~\cite{charlesworth_other_2007}.
    However, differentiating between an adaptive mutation and a beneficial \DIFdelbegin \DIFdel{back-mutation }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutation }\DIFaddend remains challenging~\cite{chi_detecting_2020}.
    Indeed, an adaptive mutation responding to a change in the environment and a beneficial \DIFdelbegin \DIFdel{back-mutation }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutation }\DIFaddend have equivalent fitness consequences for their bearer~\cite{charlesworth_other_2007}.
    ...
    On the other hand, beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend are predictable because, under a stable fitness landscape, any change from non-optimal to optimal amino acids will move back the site \DIFdelbegin \DIFdel{towards }\DIFdelend \DIFaddbegin \DIFadd{toward }\DIFaddend the equilibrium expected under the fitness landscape~\cite{moses_inferring_2009, fischer_germline_2011, chen_hunting_2021}.
    \DIFdelbegin \DIFdel{It }\DIFdelend \DIFaddbegin \DIFadd{They }\DIFaddend can then be distinguished from truly novel beneficial mutations because the latter are not expected to mutate \DIFdelbegin \DIFdel{mutate towards }\DIFdelend \DIFaddbegin \DIFadd{toward }\DIFaddend the amino acids of higher fitnesses defined by the stable fitness landscape but rather mutate to amino acids showing a diversified pattern (fig.~\ref{fig:fitness-landscape}).
    ...
    Thus, according to its $\Sphy$ value, we can classify any mutation as either a deleterious mutation toward a less fit amino acid ($\SphyDel \coloneqq \Sphy < -1$), a nearly-neutral mutation ($\SphyNeu \coloneqq -1 < \Sphy <1$)\DIFdelbegin \DIFdel{or a beneficial back-mutation }\DIFdelend \DIFaddbegin \DIFadd{, or a mutation }\DIFaddend toward a known fitter amino acid \DIFaddbegin \DIFadd{constituting thus a beneficial non-adaptive mutation }\DIFaddend ($\SphyBen \coloneqq \Sphy > 1$).

    Having identified which potential DNA changes represent beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend (fig.~\ref{fig:method}B), we retrieved polymorphism data from 28 wild and domesticated populations belonging to \DIFdelbegin \DIFdel{6 }\DIFdelend \DIFaddbegin \DIFadd{six }\DIFaddend genera (\textit{Equus}, \textit{Bos}, \textit{Capra}, \textit{Ovis}, \textit{Chlorocebus}, and \textit{Homo}) to assess the presence of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend at the population scale.
    We focused on mutations currently segregating within populations and substitutions in the terminal branches, and checked if any of these observed changes were \DIFdelbegin \DIFdel{beneficial back-mutations }\DIFdelend \DIFaddbegin \DIFadd{indeed beneficial }\DIFaddend (fig.~\ref{fig:method}C-E).
    A similar approach demonstrated the presence of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend in humans~\cite{moses_inferring_2009, fischer_germline_2011} and in plants~\cite{chen_hunting_2021}.
    ...
    The mutation-selection model used in the present work integrates phylogenetic relationships, and thus allows us to estimate the fitness landscape in shallower phylogenetic trees, and therefore can be applied almost \DIFdelbegin \DIFdel{genome-wide}\DIFdelend \DIFaddbegin \DIFadd{exome-wide}\DIFaddend ~\cite{rodrigue_mutationselection_2010}.

    We first quantified the likelihood of any DNA mutation to be a beneficial \DIFdelbegin \DIFdel{back-mutation}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutation}\DIFaddend , that is, whenever a DNA mutation increases fitness under a stable fitness landscape ($\SphyBen$).
    Subsequently, by quantifying the total amount of beneficial mutations in the current population across all types of DNA mutations ($\SphyDel$, \DIFdelbegin \DIFdel{$\SphyDel$ }\DIFdelend \DIFaddbegin \DIFadd{$\SphyNeu$ }\DIFaddend and $\SphyBen$), we could tease apart beneficial \DIFdelbegin \DIFdel{back-mutations from beneficial }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive from }\DIFaddend adaptive mutations resulting from a change in the fitness landscape.
    Altogether, in this study, by integrating large-scale genomic datasets at both phylogenetic and population scales, we propose a way to explicitly quantify the contribution of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend to positive selection across the entire exome of the six genera (figs.~\ref{fig:method}F-G).
    ...
    \DIFaddbegin \DIFadd{Because estimation $\Sphy$ was based on the assumption that the fitness landscape is stable across mammals, $\SphyBen$ mutations (i.e., with $\Sphy>1$) bring the bearer of this mutation toward an amino acid predicted to be fitter across mammals.
    }\DIFaddend Importantly, the mammalian alignment used to estimate the \DIFdelbegin \DIFdel{amino-acid }\DIFdelend \DIFaddbegin \DIFadd{amino acid }\DIFaddend fitness landscape did not include the \DIFdelbegin \DIFdel{genera for which we estimated selection at the population scale as this would bias the estimate of the fitness landscape and generate circular reasoning.
    Because $\Sphy$ was based on estimating fitness effects at the phylogenetic scale for all mammals, substitutions with $\Sphy>1$ (}\DIFdelend \DIFaddbegin \DIFadd{six focal genera and their sister species.
    ...
    Example substitutions in the terminal lineage of }\textit{\DIFadd{Chlorocebus sabaeus}} \DIFadd{which are classified as }\DIFaddend $\SphyBen$ \DIFdelbegin \DIFdel{) bring the population toward an amino acid predicted to be fitter in mammals.These changes are thus considered as beneficial back-substitutions rather than a species-specific adaptive change.}\DIFdelend \DIFaddbegin \DIFadd{are shown in supplementary materials (section S2).
    For instance, in the mammalian protein-coding DNA alignment of gene }\textit{\DIFadd{SELE}}\DIFadd{, the nucleotide at site 1722 has mutated (from T to C) at the base of Simiiformes (monkeys and apes), modifying the corresponding amino acid from Serine to Proline, but has been subsequently reverted in the branch of }\textit{\DIFadd{Chlorocebus sabaeus}} \DIFadd{(fig.~S1).
    However, other substitutions classified as $\SphyBen$ in the terminal branch of }\textit{\DIFadd{Chlorocebus sabaeus}} \DIFadd{cannot be clearly interpreted as reversions along the terminal branch, and show several transitions to this amino acid across the mammalian phylogeny, as for instance site 3145 of gene }\textit{\DIFadd{THSD7A}} \DIFadd{(fig.~S2).
}


    \DIFaddend Among all the substitutions found in each terminal branch, between 10 and 13\% were $\SphyBen$, while \DIFdelbegin \DIFdel{instead beneficial back-mutations }\DIFdelend \DIFaddbegin \DIFadd{$\SphyBen$ mutations only }\DIFaddend represent between 0.9 and 1.2\% of all non-synonymous mutations (table~S1 and fig.~\ref{fig:homo-afr-results}A-B for humans).
    \DIFdelbegin \DIFdel{In principle, beneficial back-mutations }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, since in principle, $\SphyBen$ mutations }\DIFaddend are bound to reach fixation more often than neutral mutations\DIFdelbegin \DIFdel{.
    Hence}\DIFdelend , we calculated the $\dnds$ ratio of non-synonymous over synonymous divergence for all terminal lineages, focusing on the non-synonymous changes predicted as \DIFdelbegin \DIFdel{beneficial back-mutations }\DIFdelend \DIFaddbegin \DIFadd{$\SphyBen$ mutations }\DIFaddend ($\dn(\SphyBen) / \ds$).
    We obtained values between 1.17 and 1.75 in the different lineages (table~S2), \DIFdelbegin \DIFdel{implying i) }\DIFdelend \DIFaddbegin \DIFadd{meaning }\DIFaddend that $\SphyBen$ mutations \DIFdelbegin \DIFdel{reached fixation }\DIFdelend \DIFaddbegin \DIFadd{reach fixation slightly }\DIFaddend more frequently than synonymous mutations that are supposed to be neutral, \DIFdelbegin \DIFdel{and ii) that these back-mutations are effectively }\DIFdelend \DIFaddbegin \DIFadd{consistent with these $\SphyBen$ mutations being weakly }\DIFaddend beneficial.

    This result further \DIFdelbegin \DIFdel{indicated }\DIFdelend \DIFaddbegin \DIFadd{indicates }\DIFaddend that using $\dnds$ as an estimate of adaptation is biased due to the presence of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend among the non-synonymous substitutions.
    By discarding all beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend we can obtain an estimate of $\dnds$ which is not inflated.
    By comparing these two ways of calculating $\dnds$ (see section~\ref{subsec:substitution-mapping-in-the-terminal-branch} in Materials \& Methods), we calculated that beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend inflate $\dnds$ values by between 9 and 12\% across genera\DIFdelbegin \DIFdel{while only representing }\DIFdelend \DIFaddbegin \DIFadd{. This represents a substantial increase when considering that beneficial non-adaptive mutations only represent }\DIFaddend between 0.9 and 1.2\% of non-synonymous \DIFdelbegin \DIFdel{mutations }\DIFdelend \DIFaddbegin \DIFadd{mutational opportunities }\DIFaddend (table~S3).
    ...
    Finally, mutations predicted to be \DIFdelbegin \DIFdel{beneficial back-mutations based on the selection coefficients at the phylogenetic scale (}\DIFdelend $\SphyBen$ \DIFdelbegin \DIFdel{) }\DIFdelend were indeed beneficial for individuals bearing them, with a \textit{precision} (fig.~\ref{fig:method}F) in the range of 19--87\% (table~\ref{table:proba} and fig.~\ref{fig:homo-afr-results}D for humans).
    This result confirms that selection \DIFdelbegin \DIFdel{towards }\DIFdelend \DIFaddbegin \DIFadd{toward }\DIFaddend amino acids restoring existing functions is ongoing in these populations.
    Importantly, the \textit{recall} value in this case, computed as $\proba [ \SphyBen  \given  \SpopBen]$, is the probability for a beneficial mutation at the population scale to be a \DIFdelbegin \DIFdel{beneficial back-mutation}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive}\DIFaddend , i.e., going toward a  fitter amino acid \DIFaddbegin \DIFadd{given a stable fitness landscape }\DIFaddend (fig.~\ref{fig:method}G, table~S6).
    ...
    \DIFdelbegin \DIFdel{Because }\DIFdelend \DIFaddbegin \DIFadd{Additionally, we controlled that these estimations are not affected by SNP mispolarization (figs.~S6-S7).
    Moreover, because }\DIFaddend the phylogenetic mutation-selection codon model should fit better for genes with uniformly conserved functions, as a control, we filtered out genes under pervasive adaptation~\cite{latrille_genes_2023}, and indeed we found an increase in the proportion of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{mutations that are non-adaptive }\DIFaddend ($\proba [ \SphyBen  \given  \SpopBen]$), consistent with our expectation (Wilcoxon signed-rank, $s=80$, $p=0.002$, table~S7).
    ...
    In particular, we have been able to quantify the proportion of beneficial \DIFdelbegin \DIFdel{back-mutations among all beneficial mutations , }\DIFdelend \DIFaddbegin \DIFadd{mutations that are non-adapative (i.e., not a response to a change in fitness landscape), }\DIFaddend which has only been achievable by combining \DIFdelbegin \DIFdel{genome-wide }\DIFdelend \DIFaddbegin \DIFadd{exome-wide }\DIFaddend data from both phylogenetic and population scales.
    ...
    We \DIFdelbegin \DIFdel{can compare }\DIFdelend \DIFaddbegin \DIFadd{have compared }\DIFaddend these selective effects to observations at the population level (figs.~\DIFdelbegin \DIFdel{S4-6).
    By }\DIFdelend \DIFaddbegin \DIFadd{S8-10), and by }\DIFaddend doing so, we \DIFaddbegin \DIFadd{have }\DIFaddend confirmed that mutations predicted to be deleterious ($\SphyDel \coloneqq \Sphy < -1$) are \DIFaddbegin \DIFadd{generally }\DIFaddend purified away in extant populations.
    ...
    However\DIFdelbegin \DIFdel{– }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend contrary to SIFT scores\DIFdelbegin \DIFdel{– }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend our mutation-selection model is parameterized by a fitness function such that changes are directly interpretable as fitness effects \DIFaddbegin \DIFadd{(see also section S7)}\DIFaddend .
    In this regard, an interesting prediction of our model is that \DIFdelbegin \DIFdel{back-mutations are beneficial because they revert previous deleterious changes}\DIFdelend \DIFaddbegin \DIFadd{some deleterious mutations reach fixation due to genetic drift, while beneficial non-adaptive mutations are restoring states of higher fitness}\DIFaddend .
    We have tested this hypothesis and \DIFdelbegin \DIFdel{found that these back-mutations }\DIFdelend \DIFaddbegin \DIFadd{have found that a substantial part of these predicted non-adaptive mutations }\DIFaddend ($\SphyBen \coloneqq \Sphy > 1 $) are indeed beneficial in extant populations.
    We \DIFdelbegin \DIFdel{estimated }\DIFdelend \DIFaddbegin \DIFadd{estimate }\DIFaddend that between 11 and 82\% of all beneficial mutations in mammalian populations \DIFdelbegin \DIFdel{have not been driven by adaptation but instead have reversed deleterious substitutions which have accumulated along the phylogenetic tree}\DIFdelend \DIFaddbegin \DIFadd{are not adaptive}\DIFaddend .
    More specifically, in 24 out of 28 populations analyzed, the percentage of beneficial mutations estimated to be \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive }\DIFaddend falls between 15 and 45\%.
    These results suggest that many beneficial mutations are \DIFdelbegin \DIFdel{non-adaptive }\DIFdelend \DIFaddbegin \DIFadd{not adaptive, }\DIFaddend but rather restore \DIFdelbegin \DIFdel{ancestral }\DIFdelend states of higher fitness.
    Hence, we can correctly estimate the extent of adaptive evolution only if we account for the number of beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend ~\cite{keightley_what_2010, rice_evolutionarily_2015}.
    ...
    \subsection*{\DIFdelbegin \DIFdel{Interpreting the proportion }\DIFdelend \DIFaddbegin \DIFadd{The influence }\DIFaddend of \DIFdelbegin \DIFdel{beneficial back-mutations}\DIFdelend \DIFaddbegin \DIFadd{effective population size}\DIFaddend }

    Across the genome, \DIFdelbegin \DIFdel{beneficial-back }\DIFdelend \DIFaddbegin \DIFadd{beneficial non-adaptive }\DIFaddend mutations and deleterious mutations reaching fixation create a balance in which genomes are constantly damaged and restored simultaneously at different loci \DIFaddbegin \DIFadd{due to drift}\DIFaddend .
    Since the probability of fixation of mutations depends on the effective population size ($\Ne$), the history of $\Ne$ plays a crucial role in \DIFdelbegin \DIFdel{setting }\DIFdelend \DIFaddbegin \DIFadd{determining }\DIFaddend the number of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend compensating for deleterious mutations~\cite{latrille_inferring_2021}.
    For example, a population size expansion will increase the efficacy of selection, and a larger proportion of mutations will be beneficial (otherwise effectively neutral), thus increasing the number of beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend .
    On the other hand, a population that has experienced a high $\Ne$ throughout its history should be closer to an optimal state under a stable fitness landscape, having suffered fewer fixations of deleterious mutations and therefore decreasing the probability of beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend ~\cite{huber_determining_2017}.
    Overall, we expect the proportion of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend to be more dependent on $\Ne$’s long-term expansions and contractions than on the short-term ones~\cite{charlesworth_other_2007,huber_determining_2017}.
    Accounting for phylogenetic relationships, we found no correlation between the proportion of \DIFdelbegin \DIFdel{back-mutations and }\DIFdelend \DIFaddbegin \DIFadd{beneficial non-adaptive mutations and estimates of }\DIFaddend $\Ne$ \DIFaddbegin \DIFadd{based on genetic diversity }\DIFaddend ($r^2=0.00$, $p=0.772$, fig.~\ref{fig:diversity}B)\DIFaddbegin \DIFadd{.
}\DIFaddend 

    \DIFdelbegin \DIFdel{The exact estimation of the contribution of beneficial back-mutations to positive selection relies on some hypotheses at both the phylogenetic and population scales and is sensitive to methodological limitations.
    ...
    However, adaptation is not the only factor hindering the detection of beneficial back-mutations; data quality and potentially inadequate modeling choices of both the fitness landscape and the DFE might also lead to missed predictions }\DIFdelend \DIFaddbegin \DIFadd{Moreover, because our model assumes a fixed fitness landscape, it implicitly assumes that $\Ne$ is constant along the phylogenetic tree.
...
, our estimated proportion of beneficial non-adaptive mutations among adaptive ones is likely to be an underestimation}\DIFaddend .

    \DIFaddbegin \subsection*{\DIFadd{The role of epistasis and compensatory mutations}}

    \DIFaddend Our model assumes that amino-acid fitness landscapes are site-specific and also independent of one another, whereas under pervasive epistasis, the fitness effect of any mutation at a particular site would depend on the amino acids present at other sites.
    ...
    Epistasis therefore allows for compensatory mutations, which restore fitness through mutations at loci different from where deleterious mutations took place\DIFdelbegin \DIFdel{.
    It therefore, represents another case case }\DIFdelend \DIFaddbegin \DIFadd{, representing another case }\DIFaddend of non-adaptive beneficial mutations, but one which is not accounted for by our method.
    \DIFdelbegin \DIFdel{Therefore}\DIFdelend \DIFaddbegin \DIFadd{Hence}\DIFaddend , the beneficial mutations that we classify as putatively adaptive might in fact be compensatory mutations, making our estimation of the rate of non-adaptive beneficial mutations conservative.

    Despite epistasis being an important factor in protein evolution, several \DIFdelbegin \DIFdel{deep mutational scanning laboratory }\DIFdelend \DIFaddbegin \DIFadd{deep-mutational scanning }\DIFaddend experiments have revealed that a site-specific fitness landscape \DIFdelbegin \DIFdel{is a reasonable approximation of the fitness landscape of proteins, and even }\DIFdelend predicts the evolution of sequences in nature \DIFaddbegin \DIFadd{with considerable accuracy}\DIFaddend ~\cite{ashenberg_mutational_2013, doud_sitespecific_2015, bloom_identification_2017}.
    Additionally, the fact that we observe such a high proportion of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend suggests that the underlying assumptions of our model, namely site-independence, implying no epistasis, and a static fitness landscape, are a reasonable approximation for the underlying fitness landscape of proteins.
    ...
    \DIFdelbegin \DIFdel{Moreover, because our model assumes a fixed landscape, it implicitly assumes that $\Ne$ is constant along the phylogenetic tree.
    Fluctuations due to changes in }\DIFdelend \DIFaddbegin \subsection*{\DIFadd{Other assumptions and methodological limitations}}
    ...
    Indeed, data quality and potentially inadequate modeling choices of both }\DIFaddend the fitness landscape \DIFdelbegin \DIFdel{or in $\Ne$ will be averaged out by the assumption of the current model that $\Ne$ is constant across lineages.
    It was recently shown~\mbox{%DIFAUXCMD
\cite{latrille_inferring_2021}}\hskip0pt%DIFAUXCMD
, using highly computer intensive mutation-selection models with fluctuating $\Ne$, that relaxing the assumption of a constant $\Ne$ results in more extreme estimates of amino-acid fitnesses than with the standard model used in this study.
    ...
    As a consequence, mutations that would otherwise fall into }\DIFdelend \DIFaddbegin \DIFadd{(at the phylogenetic scale) and fitness effects (at the population scale) might also lead to missed prediction~\mbox{%DIFAUXCMD
\cite{moutinho_variation_2019}}\hskip0pt%DIFAUXCMD
.
    To be conservative, we considered that any mutation under positive selection at the population scale (}\DIFaddend $\SpopBen$\DIFdelbegin \DIFdel{or $\SpopDel$ will be classified as $\SpopNeu$, resulting ultimately in lower estimates of the proportion of beneficial back-mutations ($\proba [ \SphyBen  \given  \SpopBen]$) .
    Given this inflation of missed predictions due to change in population sizes~\mbox{%DIFAUXCMD
\cite{lanfear_population_2014, jones_shifting_2017, platt_protein_2018}}\hskip0pt%DIFAUXCMD
, our estimated proportion of beneficial back-mutations among adaptive ones is likely to therefore be an underestimation}\DIFdelend \DIFaddbegin \DIFadd{) but not predicted as such at the phylogenetic scale (not $\SphyBen$) is potentially an adaptation}\DIFaddend .
    In practice, \DIFdelbegin \DIFdel{advantageous mutations can be depleted in the $\SphyNeu$ and $\SphyDel$ class in some species but pervasive in others, }\DIFdelend \DIFaddbegin \DIFadd{we obtained different values of the proportion of non-adaptive beneficial mutations }\DIFaddend depending on the model used to infer the \DIFdelbegin \DIFdel{DFE }\DIFdelend \DIFaddbegin \DIFadd{fitness effects }\DIFaddend at the population scale\DIFaddbegin \DIFadd{, and the filtering or not of CpG mutations~\mbox{%DIFAUXCMD
\cite{eyre-walker_how_2014} }\hskip0pt%DIFAUXCMD
}\DIFaddend (tables~\DIFdelbegin \DIFdel{S8-12}\DIFdelend \DIFaddbegin \DIFadd{S8-14}\DIFaddend ).
    It appears that our estimation can be \DIFdelbegin \DIFdel{quite }\DIFdelend sensitive to model misspecification and overall, while we provide an order of magnitude for the contribution of beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend to positive selection, \DIFdelbegin \DIFdel{further methodological development }\DIFdelend \DIFaddbegin \DIFadd{methodological improvement }\DIFaddend on the estimation of the distribution of fitness effects is needed to increase the \DIFdelbegin \DIFdel{confidence on }\DIFdelend \DIFaddbegin \DIFadd{precision of }\DIFaddend this value.

    \DIFaddbegin \DIFadd{Importantly, if there are several substitutions toward the same amino acid along the mammalian tree, (see fig.~S2) our mutation-selection model cannot distinguish between a scenario where mammals have fixed deleterious mutations that are reverted in several lineages, from more complex scenarios involving adaptation.
    ...
    \DIFaddend \subsection*{Detecting adaptation above the nearly-neutral background}
...
\cite{kimura_evolutionary_1968,jensen_importance_2019,gillespie_substitution_1994,ohta_nearly_1992}}\hskip0pt%DIFAUXCMD
    ...
    Here we provide evidence that in mammalian orthologs, many substitutions occur through fixation of both deleterious mutations and beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend .
    However, detecting adaptation above this background of nearly-neutral substitutions remains a \DIFdelbegin \DIFdel{central question}\DIFdelend \DIFaddbegin \DIFadd{challenge}\DIFaddend ~\cite{kimura_evolutionary_1968,ohta_development_1996}.
    ...
    However, as shown in this study, a $\dnds > 1$ is not necessarily a signature of adaptation but can be due to beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend .
    ...
    \DIFaddbegin \subsection*{\DIFadd{Conclusions}}

    \DIFaddend Here, we have provided empirical evidence that an evolutionary model assuming a stable fitness landscape at the mammalian scale allows us to predict the fitness effects of mutations in extant populations and individuals, acknowledging the balance between deleterious and beneficial \DIFdelbegin \DIFdel{back-mutations}\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations}\DIFaddend .
    We argue that such a model \DIFdelbegin \DIFdel{represents }\DIFdelend \DIFaddbegin \DIFadd{would represent }\DIFaddend a null expectation for the evolution of protein-coding genes in the absence of adaptation.
    \DIFaddbegin \DIFadd{Altogether, because a substantial part of positive selection can be explained by beneficial non-adaptive mutations, but not its entirety, we argue that the mammalian exome is shaped by both adaptive and non-adaptive processes, and that none of them alone is sufficient to explain the observed patterns of changes.
    }\DIFaddend In that sense, to avoid conflating beneficial mutations with adaptive evolution, the term ``adaptation`` should retain its original meaning associated with a change in the underlying fitness landscape and be modelled as such.
    ...
    \textit{Precision} is the proportion of mutations correctly predicted as beneficial ($\proba [ \SpopBen \cap  \SphyBen]$) out of all predicted as beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend ($\proba [ \SphyBen]$), which can be written as a conditional probability:

    \begin{equation}
        \frac{\proba [ \SpopBen  \cap  \SphyBen]}{\proba [ \SphyBen]} = \proba [ \SpopBen  \given  \SphyBen].
        \label{eq:precision}
    \end{equation}
    Namely, \textit{precision} corresponds to the probability for a \DIFdelbegin \DIFdel{back-mutation (}\DIFdelend $\SphyBen$ \DIFdelbegin \DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{mutation }\DIFaddend to be effectively beneficial at the population level ($\SpopBen$).
    This probability, computed from eq.~\ref{eq:polyProbaAdv}, is obtained by restricting our analysis to SNPs that are predicted to be beneficial \DIFdelbegin \DIFdel{back-mutations }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutations }\DIFaddend (yellow fill for the category $\SphyBen$ in fig.~\ref{fig:homo-afr-results}D).
    ...
    \end{equation}
    Namely, \textit{recall} corresponds to the probability for a beneficial mutation at the population level ($\SpopBen$) to be a beneficial \DIFdelbegin \DIFdel{back-mutation }\DIFdelend \DIFaddbegin \DIFadd{non-adaptive mutation }\DIFaddend ($\SphyBen$).
    ...
    The data underlying this article are available at \DIFdelbegin %DIFDELCMD < \url{https://doi.org/10.5281/zenodo.7878954}%%%
...
\newpage
    \begin{figure*}[!htb]
        ...
        \caption{
            ...
            Under a stable fitness landscape (B), most mutations reaching fixation are either slightly deleterious reaching fixation due to drift or are beneficial \DIFdelbeginFL \DIFdelFL{back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-adaptive mutations }\DIFaddendFL restoring a more optimal amino acid.
            At the phylogenetic scale (D), deleterious substitutions (negative signs) are often reverted via beneficial \DIFdelbeginFL \DIFdelFL{back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-adaptive mutations }\DIFaddendFL (positive signs), promoting phenotype stability and preserving well-established biological systems.
            Even though, individually, any \DIFdelbeginFL \DIFdelFL{back-mutation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{beneficial non-adaptive mutation }\DIFaddendFL might have a \DIFdelbeginFL \DIFdelFL{small beneficial }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{weak }\DIFaddendFL effect on its bearer, we expect \DIFdelbeginFL \DIFdelFL{beneficial back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{them }\DIFaddendFL to be scattered across the genome and the genome-wide signature of beneficial \DIFdelbeginFL \DIFdelFL{back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-adaptive mutations }\DIFaddendFL to be detectable and quantifiable.}
    ...
    \begin{figure*}[!h]
        ...
        \caption{
            ...
            Depending on $\Sphy$ (B) mutations can be predicted as deleterious ($\SphyDel$), nearly-neutral ($\SphyNeu$) or beneficial \DIFdelbeginFL \DIFdelFL{back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-adaptive mutations }\DIFaddendFL ($\SphyBen$) toward a fitter amino acid and repairing existing functions.
            ...
            The \textit{recall} value for class $\SphyBen$ is the probability \DIFdelbeginFL \DIFdelFL{of back-mutations among all }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for }\DIFaddendFL beneficial \DIFdelbeginFL \DIFdelFL{ones }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{mutations to be non-adaptive }\DIFaddendFL (G).
        }
        \label{fig:method}
    \end{figure*}
    ...
    \begin{figure*}[!ht]
        ...
        \caption{
            ...
            (B) Proportion of beneficial \DIFdelbeginFL \DIFdelFL{back-mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-adaptive mutations }\DIFaddendFL among all beneficial mutations ($\proba [ \SphyBen  \given  \SpopBen]$ in the y-axis), shown as a function of $\Ne$ in x-axis.
