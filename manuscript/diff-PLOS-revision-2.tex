...
%DIF DEL main-PLOS-revision-plos2015.tex     Thu Dec  5 11:42:43 2024
%DIF ADD main-PLOS-revision-2-plos2015.tex   Fri Dec  6 15:50:04 2024
...
\caption{
\DIFaddbeginFL {\bf \DIFaddFL{Changing and stable fitness landscapes.}}
\DIFaddendFL (A \& B) For a given codon position of a protein-coding DNA sequence, amino acids (x-axis) have different fitness values (y-axis).
...
For instance, in the mammalian protein-coding DNA alignment of gene \textit{SELE}, the nucleotide at site 1722 has mutated (from T to C) at the base of Simiiformes (monkeys and apes), modifying the corresponding amino acid from Serine to Proline, but has been subsequently reverted in the branch of \textit{Chlorocebus sabaeus} (\DIFdelbegin \DIFdel{fig.~S2}\DIFdelend \DIFaddbegin \nameref{S2_Fig}\DIFaddend ).
However, other substitutions classified as $\SphyBen$ in the terminal branch of \textit{Chlorocebus sabaeus} cannot be clearly interpreted as reversions along the terminal branch, and show several transitions to this amino acid across the mammalian phylogeny, as for instance site 3145 of gene \textit{THSD7A} (\DIFdelbegin \DIFdel{fig.~S3}\DIFdelend \DIFaddbegin \nameref{S3_Fig}\DIFaddend ).

Among all the substitutions found in each terminal branch, between 10 and 13\% were $\SphyBen$, while $\SphyBen$ mutations only represent between 0.9 and 1.2\% of all non-synonymous mutations (\DIFdelbegin \DIFdel{table~S1 and fig.~}\DIFdelend \DIFaddbegin \DIFadd{Fig~}\DIFaddend \ref{fig:homo-afr-results}A-B for humans \DIFaddbegin \DIFadd{and ~}\nameref{S1_Table} \DIFadd{for all dataset}\DIFaddend ).
...
This rate is highly consistent across lineages (\DIFdelbegin \DIFdel{table~S1}\DIFdelend \DIFaddbegin \nameref{S1_Table}\DIFaddend ) and suggests an overall frequency of nearly neutral substitutions due to consistent long-term selection pressures, mutation and drift of approximately 20\% of all substitutions (20\%-26\% across species).
...
\caption{
\DIFaddbeginFL {\bf \DIFaddFL{Selection coefficients of mutations in humans of African descent.}}
\DIFaddendFL (A) Distribution of scaled selection coefficients ($\Sphy$), predicted for all possible non-synonymous DNA mutations away from the ancestral human exome (section \ref{subsec:nunber-of-sites}).
...
We obtained values between 1.17 and 1.75 in the different lineages (\DIFdelbegin \DIFdel{table~S2}\DIFdelend \DIFaddbegin \nameref{S2_Table}\DIFaddend ), meaning that $\SphyBen$ mutations reach fixation slightly more frequently than synonymous mutations that are supposed to be neutral, consistent with these $\SphyBen$ mutations being weakly beneficial.
Such an observation is consistent with the premise that $\SphyBen$ mutations are weakly beneficial, translating to a scaled selection coefficient between 0.32 and 1.24 (\DIFdelbegin \DIFdel{table~S2}\DIFdelend \DIFaddbegin \nameref{S2_Table}\DIFaddend ).
...
This represents a substantial increase when considering that beneficial non-adaptive mutations only represent between 0.9 and 1.2\% of non-synonymous mutational opportunities (\DIFdelbegin \DIFdel{table~S3}\DIFdelend \DIFaddbegin \nameref{S3_Table}\DIFaddend ).
...
First, SNPs classified as $\SphyBen$ are spread across the genomes and not strongly associated to the ontology terms of their respective genes (\DIFdelbegin \DIFdel{table~S4,fig.~S4}\DIFdelend \DIFaddbegin \nameref{S4_Table}\DIFadd{,~}\nameref{S4_Fig}\DIFaddend ).
...
Our results show that SNPs predicted as deleterious are associated with clinical terms such as \textit{Likely Pathogenic} and \textit{Pathogenic}, implying that, in general, the selective pressure of a mutation exerted across mammals is also predictive of its clinical effect in humans (\DIFdelbegin \DIFdel{table~S5}\DIFdelend \DIFaddbegin \nameref{S5_Table}\DIFaddend )~\cite{sullivan_leveraging_2023}.
Conversely, $\SphyBen$ mutations are associated with clinical terms such as \textit{Benign} and \textit{Likely Benign}, which shows that $\SphyBen$ mutations are less likely to be functionally damaging (\DIFdelbegin \DIFdel{table~S6}\DIFdelend \DIFaddbegin \nameref{S6_Table}\DIFaddend ).
...
For instance, deleterious SNPs usually segregate at lower frequencies because of purifying selection, which tends to remove them from the population (\DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:homo-afr-results}C for humans).
...
From these estimated fitness effects, we can derive the proportion of \DIFdelbegin \DIFdel{beneficial mutations ($\ProbaPopBen$}\DIFdelend \DIFaddbegin \DIFadd{deleterious mutations ($\ProbaPopDel$,~}\nameref{S5_Fig}\DIFaddend ), nearly-neutral mutations ($\ProbaPopNeu$\DIFaddbegin \DIFadd{,~}\nameref{S6_Fig}\DIFadd{) and beneficial mutations ($\ProbaPopBen$,~}\nameref{S7_Fig}\DIFaddend ) \DIFdelbegin \DIFdel{and deleterious mutations ($\ProbaPopDel$) }\DIFdelend at the population scale (see section~\ref{subsec:s-polymorphism-method} in Materials \& Methods\DIFdelbegin \DIFdel{, figs.~S5-S8}\DIFdelend ).
These approaches offer a unique opportunity to contrast selection coefficients estimated at the \DIFdelbegin \DIFdel{population scale ($\Spop$}\DIFdelend \DIFaddbegin \DIFadd{phylogenetic scale ($\Sphy$}\DIFaddend ) and at the \DIFdelbegin \DIFdel{phylogenetic scale ($\Sphy$) }\DIFdelend \DIFaddbegin \DIFadd{population scale ($\Spop$) in different dataset (}\nameref{S8_Fig}\DIFadd{)}\DIFaddend .

Across our selection classes ($\SphyDel$, $\SphyNeu$ and $\SphyBen$), one can ultimately estimate the proportion of correct and incorrect predictions, leading to an estimation of \textit{precision} and \textit{recall} (\DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:method}F-G and section~\ref{subsec:precisison_recall} in Materials \& Methods).
Across 28 populations of different mammal species, mutations predicted to be deleterious at the phylogenetic scale ($\SphyDel$) were indeed purged at the population scale, with a \textit{precision} in the range of 90--97\% (\DIFdelbegin \DIFdel{table}\DIFdelend \DIFaddbegin \DIFadd{Table}\DIFaddend ~\ref{table:proba} and \DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:homo-afr-results}D for humans).
Conversely, a \textit{recall} in the range of 96--100\% implied that mutations found to be deleterious at the population scale were most likely also predicted to be deleterious at the phylogenetic scale (\DIFdelbegin \DIFdel{table}\DIFdelend \DIFaddbegin \DIFadd{Table}\DIFaddend ~\ref{table:proba}).
...
\centering
\caption{
\DIFaddbeginFL {\bf \textit{\DIFaddFL{Precision}} \DIFaddFL{and }\textit{\DIFaddFL{recall}} \DIFaddFL{for estimated selection coefficient of mutations given by mutation-selection models ($\bm{\Sphy}$).}}\DIFaddendFL }
\scriptsize
...
\end{tabular}
\normalsize
\begin{flushleft} \textit{Precision} \DIFdelbeginFL \DIFdelFL{and }\textit{\DIFdelFL{recall}} %DIFAUXCMD
...
Using correlation analyses that accounted for phylogenetic relationship (see section\DIFaddbegin \DIFadd{~}\DIFaddend \ref{subsec:correlation-diversity} in Materials \& Methods\DIFaddbegin \DIFadd{,~}\nameref{S9_Fig}\DIFaddend ), we found that higher $\Ne$ was associated with a smaller proportion of nearly-neutral mutations ($r^2=0.31$, $p=0.001$, \DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:diversity}A).
This result follows the prediction of the nearly-neutral theory and suggests that in populations with higher diversity (e.g.,~\textit{Bos} or \textit{Ovis}), discrimination between beneficial and deleterious mutations is more likely to occur (\DIFdelbegin \DIFdel{figs.~S9-S12}\DIFdelend \DIFaddbegin \nameref{S10_Fig}\DIFadd{,~}\nameref{S11_Fig}\DIFadd{,~}\nameref{S12_Fig}\DIFaddend ).
...
\caption{
\DIFaddbeginFL {\bf \DIFaddFL{Proportion of nearly-neutral mutations and beneficial non-adaptive mutations as a function of effective population size ($\Ne$).}}
\DIFaddendFL Populations in circles, mean of the species across the populations as squares.
...
Finally, mutations predicted to be $\SphyBen$ were indeed beneficial for individuals bearing them, with a \textit{precision} (\DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:method}F) in the range of 19--87\% (\DIFdelbegin \DIFdel{table}\DIFdelend \DIFaddbegin \DIFadd{Table}\DIFaddend ~\ref{table:proba} and \DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:homo-afr-results}D for humans).
This result confirms that selection toward amino acids restoring existing functions is ongoing in these populations.
Importantly, the \textit{recall} value in this case, computed as $\proba [ \SphyBen \given \SpopBen]$, is the probability for a beneficial mutation at the population scale to be a non-adaptive, i.e., going toward a fitter amino acid given a stable fitness landscape (\DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:method}G,\DIFdelbegin \DIFdel{table~S7}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S7_Table}\DIFaddend ).
...
Across the 28 populations, this proportion is in the range of 11--82\% (\DIFdelbegin \DIFdel{table}\DIFdelend \DIFaddbegin \DIFadd{Table}\DIFaddend ~\ref{table:proba}), with a mean of 30\%.
Accounting for phylogenetic relationships, we found no correlation between the proportion of beneficial non-adaptive mutations and estimates of $\Ne$ based on genetic diversity ($r^2=0.00$, $p=0.772$, \DIFdelbegin \DIFdel{fig.}\DIFdelend \DIFaddbegin \DIFadd{Fig}\DIFaddend ~\ref{fig:diversity}B).

We additionally performed controls and simulations to ensure our results were robust.
First, we controlled that these estimations were not affected by SNP mispolarization (\DIFdelbegin \DIFdel{figs.~S13-S14}\DIFdelend \DIFaddbegin \nameref{S13_Fig}\DIFadd{,~}\nameref{S14_Fig}\DIFaddend ).
Second, we performed simulations at the population-genetic level and confirmed that our method was able to recover the proportion of beneficial mutations that are non-adaptive in synthetic polymorphism datasets (\DIFdelbegin \DIFdel{fig.~S15}\DIFdelend \DIFaddbegin \nameref{S15_Fig}\DIFaddend ).
Third, we ran our analysis filtering out CpG mutations and obtained values of $\proba [ \SphyBen \given \SpopBen]$ in the range of 5--27\%, with a mean of 14\% (\DIFdelbegin \DIFdel{tables~S8-9 and fig.~S16-17}\DIFdelend \DIFaddbegin \nameref{S8_Table}\DIFadd{, }\nameref{S9_Table}\DIFadd{, }\nameref{S16_Fig}\DIFaddend ), providing more conservative estimates.
...
In this subset of the exome, containing genes with a more stable fitness landscape, we found an increase in the proportion of beneficial mutations that are non-adaptive (Wilcoxon signed-rank, $s=80$, $p=0.002$,\DIFdelbegin \DIFdel{table~S10}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S10_Table}\DIFaddend ), consistent with our expectation that beneficial mutations occur more frequently in genes under changing fitness landscapes.
...
Hence, we estimated \textit{precision} and \textit{recall} as done previously, but now including the number of substitutions per site as input for the DFE estimators (see\DIFaddbegin \DIFadd{~}\DIFaddend \ref{subsec:s-polymorphism-method} in Materials \& Methods and\DIFdelbegin \DIFdel{fig.~S18}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S17_Fig}\DIFaddend ).
When including substitutions in the terminal lineage, estimates of $\proba [ \SphyBen \given \SpopBen]$ are in the 10--78\% range with a mean of 36\%\DIFdelbegin \DIFdel{(table~S11-12)}\DIFdelend , and 19 out of 28 estimates fall between 15\% and 45\% \DIFaddbegin \DIFadd{(}\nameref{S11_Table}\DIFadd{, }\nameref{S12_Table}\DIFadd{)}\DIFaddend .

Additionally, we controlled that these estimations were not affected by SNP mispolarization (\DIFdelbegin \DIFdel{fig.~S19}\DIFdelend \DIFaddbegin \nameref{S18_Fig}\DIFaddend ).
We also filtered out genes under pervasive adaptation, and again found an increase in $\proba [ \SphyBen \given \SpopBen]$, consistent with our expectation (Wilcoxon signed-rank, $s=120$, $p=0.027$,\DIFdelbegin \DIFdel{table~S13}\DIFdelend \DIFaddbegin \DIFadd{~}\nameref{S13_Table}\DIFaddend ).
...
These disjoint estimates are well correlated, with a goodness of fit $r^2={0.95, 0.89, 0.82}$ for \DIFaddbegin \DIFadd{respectively }\DIFaddend $\proba [\SpopDel]$ \DIFaddbegin \DIFadd{(}\nameref{S19_Fig}\DIFadd{)}\DIFaddend , $\proba [\SpopNeu]$\DIFaddbegin \DIFadd{(}\nameref{S20_Fig}\DIFadd{) }\DIFaddend and $\proba [\SpopBen]$ \DIFdelbegin \DIFdel{, respectively (figs.~S20-22}\DIFdelend \DIFaddbegin \DIFadd{(}\nameref{S21_Fig}\DIFaddend ).
...
Instead of using such a continuous DFE, we also tested our prediction with a non-parametric functional form for the DFE, obtaining estimates of $\proba [ \SphyBen \given \SpopBen]$ in the 8--94\% range, with a mean of 43\% (\DIFdelbegin \DIFdel{tables~S14-15}\DIFdelend \DIFaddbegin \nameref{S14_Table}\DIFadd{, }\nameref{S15_Table}\DIFaddend ).
...
If there are several substitutions toward the same amino acid along the mammalian tree (\DIFdelbegin \DIFdel{see fig.~S3}\DIFdelend \DIFaddbegin \nameref{S3_Fig}\DIFaddend ), our mutation-selection model cannot formally distinguish between a scenario where mammals have fixed deleterious mutations that are reverted in several lineages, from more complex scenarios involving convergent adaptation across mammals.
...
Importantly, we would usually expect adaptive convergent mutations to be linked to particular converging phenotypes across mammals, and hence, they should not massively affect the whole genome as we find (\DIFdelbegin \DIFdel{table~S4,fig. S4}\DIFdelend \DIFaddbegin \nameref{S4_Table}\DIFadd{,~}\nameref{S4_Fig}\DIFaddend ).
Moreover, after filtering out genes usually associated to recurrent adaptation (e.g. immune genes), we recover an even higher proportion of beneficial non-adaptive mutations (\DIFdelbegin \DIFdel{table~S8}\DIFdelend \DIFaddbegin \nameref{S8_Table}\DIFaddend ).
...
\DIFadd{This work was funded by Faculté de Biologie et de Médecine, }\DIFaddend Université de Lausanne \DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{(https://www.unil.ch; to TL, DAH and NS) and }\DIFaddend Agence Nationale de la Recherche \DIFdelbegin \DIFdel{, Grant }\DIFdelend \DIFaddbegin \DIFadd{(https://anr.fr/; grant }\DIFaddend ANR-19-CE12-0019 / HotRec \DIFaddbegin \DIFadd{to JJ). The funders did not play any role in the study design, data collection and analysis, decision to publish, or preparation of the manuscript}\DIFaddend .
...
The authors \DIFdelbegin \DIFdel{declare no conflicts of interest}\DIFdelend \DIFaddbegin \DIFadd{have declared that no competing interests exist}\DIFaddend .
...
\label{S1_File}
{\bf \DIFadd{Supplementary appendix.}} \DIFadd{Contains 43 pages of supplementary information: model parameterization, supplementary figures (S1 Fig to S21 Fig) and supplementary tables (S1 Table to S15 Table).
}

\subsection*{\DIFadd{List of Figures}}
\paragraph*{\DIFadd{S1 Fig.}}
\label{S1_Fig}
{\bf \DIFadd{Bayesian hierarchical model.}}

\paragraph*{\DIFadd{S2 Fig.}}
\label{S2_Fig}
{\bf \DIFadd{Examples of $\bm{\SphyBen}$ mutation sites in }\textit{\DIFadd{Chlorocebus sabaeus}} \DIFadd{(reversions).}}

\paragraph*{\DIFadd{S3 Fig.}}
\label{S3_Fig}
{\bf \DIFadd{Examples of $\bm{\SphyBen}$ mutation sites in }\textit{\DIFadd{Chlorocebus sabaeus}}\DIFadd{.}}

\paragraph*{\DIFadd{S4 Fig.}}
\label{S4_Fig}
{\bf \DIFadd{Enrichment in $\bm{\SphyBen}$ SNPs across all gene ontology terms.}}

\paragraph*{\DIFadd{S5 Fig.}}
\label{S5_Fig}
{\bf \DIFadd{Estimation of selection at the population scale for $\bm{\SphyDel}$ mutations.}}

\paragraph*{\DIFadd{S6 Fig.}}
\label{S6_Fig}
{\bf \DIFadd{Estimation of selection at the population scale for $\bm{\SphyNeu}$ mutations.}}

\paragraph*{\DIFadd{S7 Fig.}}
\label{S7_Fig}
{\bf \DIFadd{Estimation of selection at the population scale for $\bm{\SphyBen}$ mutations.}}

\paragraph*{\DIFadd{S8 Fig.}}
\label{S8_Fig}
{\bf \DIFadd{Distribution of scaled selection coefficients ($\bm{\Sphy}$).}}

\paragraph*{\DIFadd{S9 Fig.}}
\label{S9_Fig}
{\bf \DIFadd{Tree used for PGLM (Phylogenetic Generalized Linear Model).}}

\paragraph*{\DIFadd{S10 Fig.}}
\label{S10_Fig}
{\bf \DIFadd{Relationship between the proportion of deleterious mutations ($\bm{\SpopDel}$) and effective population size.}}

\paragraph*{\DIFadd{S11 Fig.}}
\label{S11_Fig}
{\bf \DIFadd{Relationship between the proportion of deleterious mutations ($\bm{\SpopNeu}$) and effective population size.}}

\paragraph*{\DIFadd{S12 Fig.}}
\label{S12_Fig}
{\bf \DIFadd{Relationship between the proportion of deleterious mutations ($\bm{\SpopBen}$) and effective population size.}}

\paragraph*{\DIFadd{S13 Fig.}}
\label{S13_Fig}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ as a function of distance to the sister species.}}

\paragraph*{\DIFadd{S14 Fig.}}
\label{S14_Fig}
{\bf \DIFadd{Ratio of $\bm{\proba[\Sphy]}$ over $\bm{\proba[-\Sphy]}$ for polymorphisms of the African population of }\textit{\DIFadd{Homo sapiens}}\DIFadd{.}}

\paragraph*{\DIFadd{S15 Fig.}}
\label{S15_Fig}
{\bf \DIFadd{Proportion of non-adaptive mutations among beneficial ones ($\bm{\proba{[}\SphyBen\given \SpopBen {]}}$) estimated on synthetic site-frequency spectra.}}

\paragraph*{\DIFadd{S16 Fig.}}
\label{S16_Fig}
{\bf \DIFadd{Site-frequency spectrum when masking CpG transitions.}}

\paragraph*{\DIFadd{S17 Fig.}}
\label{S17_Fig}
{\bf \DIFadd{Including substitutions in the terminal lineage to estimate $\bm{\Spop}$.}}

\paragraph*{\DIFadd{S18 Fig.}}
\label{S18_Fig}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when including divergence to estimate $\bm{\Spop}$, as a function of distance to the sister species.}}

\paragraph*{\DIFadd{S19 Fig.}}
\label{S19_Fig}
{\bf \DIFadd{$\bm{\proba [ \SpopDel ]}$ when fitting a single or three DFEs.}}

\paragraph*{\DIFadd{S20 Fig.}}
\label{S20_Fig}
{\bf \DIFadd{$\bm{\proba [ \SpopNeu ]}$ when fitting a single or three DFEs.}}

\paragraph*{\DIFadd{S21 Fig.}}
\label{S21_Fig}
{\bf \DIFadd{$\bm{\proba [ \SpopBen ]}$ when fitting a single or three DFEs.}}


\newpage
\subsection*{\DIFadd{List of Tables}}
\paragraph*{\DIFadd{S1 Table.}}
\label{S1_Table}
{\bf \DIFadd{Probability of mutations and substitutions to be $\bm{\SphyDel}$, $\bm{\SphyNeu}$ or $\bm{\SphyBen}$.}}

\paragraph*{\DIFadd{S2 Table.}}
\label{S2_Table}
{\bf \DIFadd{$\bm{\dnds}$ for $\bm{\SphyDel}$, $\bm{\SphyNeu}$ or $\bm{\SphyBen}$.}}

\paragraph*{\DIFadd{S3 Table.}}
\label{S3_Table}
{\bf \DIFadd{$\bm{\dnds}$ over-estimation due to non-adaptive beneficial mutations.}}

\paragraph*{\DIFadd{S4 Table.}}
\label{S4_Table}
{\bf \DIFadd{Gene ontology enrichment in $\bm{\SphyBen}$ SNPs.}}

\paragraph*{\DIFadd{S5 Table.}}
\label{S5_Table}
{\bf \DIFadd{Terms associated with deleterious mutations $\bm{\SphyDel}$.}}

\paragraph*{\DIFadd{S6 Table.}}
\label{S6_Table}
{\bf \DIFadd{Terms associated with non-adaptive beneficial mutations $\bm{\SphyBen}$.}}

\paragraph*{\DIFadd{S7 Table.}}
\label{S7_Table}
{\bf \DIFadd{Probability for beneficial mutations to be non-adaptive ($\bm{\proba[\SphyBen\given \SpopBen ]}$).}}

\paragraph*{\DIFadd{S8 Table.}}
\label{S8_Table}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when masking CpG transitions.}}

\paragraph*{\DIFadd{S9 Table.}}
\label{S9_Table}
{\bf \DIFadd{Precision and recall when masking CpG transitions.}}

\paragraph*{\DIFadd{S10 Table.}}
\label{S10_Table}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ excluding or not genes under adaptation.}}

\paragraph*{\DIFadd{S11 Table.}}
\label{S11_Table}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when including divergence to estimate $\bm{\Spop}$.}}

\paragraph*{\DIFadd{S12 Table.}}
\label{S12_Table}
{\bf \DIFadd{Precision and recall when including divergence to estimate $\bm{\Spop}$.}}

\paragraph*{\DIFadd{S13 Table.}}
\label{S13_Table}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ excluding or not genes under adaptation, when including divergence to estimate $\bm{\Spop}$.}}

\paragraph*{\DIFadd{S14 Table.}}
\label{S14_Table}
{\bf \DIFadd{$\bm{\proba[\SphyBen\given \SpopBen ]}$ when using a discrete DFE to estimate $\bm{\Spop}$.}}

\paragraph*{\DIFadd{S15 Table.}}
\label{S15_Table}
{\bf \DIFadd{Precision and recall when using a discrete DFE to estimate $\bm{\Spop}$.}}


\DIFaddend % Either type in your references using
...
\item%DIFAUXCMD
\DIFdel{S2: Examples sites of $\SphyBen$ mutations in }\textit{\DIFdel{Chlorocebus sabaeus}} %DIFAUXCMD
\DIFdel{(reversions) (p.10).
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{S3: Examples sites of $\SphyBen$ mutations in }\textit{\DIFdel{Chlorocebus sabaeus}} %DIFAUXCMD
\DIFdel{(p.11).
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
...
\item%DIFAUXCMD
\DIFdel{S14: Ratio of $\proba[\Sphy]$ over $\proba[-\Sphy]$ for polymorphisms of the African population of }\textit{\DIFdel{Homo sapiens}} %DIFAUXCMD
\DIFdel{(p.27).
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
...
\item%DIFAUXCMD
\DIFdel{S16: Site-frequency spectrum in African population of }\textit{\DIFdel{Homo sapiens}}%DIFAUXCMD
\DIFdel{, masking CpG transitions  (p.32).
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{S17: Site-frequency spectrum in South African population of }\textit{\DIFdel{Chlorocebus sabaeus}}%DIFAUXCMD
\DIFdel{, masking CpG transitions (p.32).
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
